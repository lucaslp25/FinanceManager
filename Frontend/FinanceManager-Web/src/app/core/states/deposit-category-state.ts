...
import { DepositCategoryResponseDTO, DepositCategoryService } from '../services/deposit-category-service';
import { tap } from 'rxjs';

@Injectable({
  providedIn: 'root',
})
export class DepositCategoryState {

  private service = inject(DepositCategoryService);
  
  private _categories = signal<DepositCategoryResponseDTO[]>([]);
  ...
  public load(){
    ...
    return this.service.loadDepositCategories().subscribe({
      next: (data) =>{
        console.log('Deposit categories loaded! ', data);
        this._categories.set(data);
        this.loaded = true;
      },
      error: (err) => console.error('Error in load the Deposit categories. ', err)
    })
  }
